...
def create_exp_name(args):
    return 'FPN-binloss{:d}-nve{:02d}-pve{:02d}-td{:d}-steps{:d}-tmreg{:d}-tmcls{:d}-convlstm{:d}-tl{:d}-sfl{:d}-ph{:d}-{:d}x{:d}-{:01d}-{:s}-{:s}-hl{:01d}s{:01d}-bn{:d}f{:d}b{:d}-bs{:02d}-{:s}-lr{:06d}-{:s}'.format(
                                            int(args.bin_loss), int(args.negative_threshold * 10), int(args.positive_threshold * 10),
                                            int(args.time_distributed_backbone), args.temporal_slice_timesteps, int(args.append_reg_temporal_net), int(args.append_cls_temporal_net),
